...
+ (id)serviceRequestMethod:(NSString *)strMethod
               SoapMessage:(NSString *)soapMsg
{
    return [[[self alloc]initWithMethod:strMethod SoapMessage:soapMsg]autorelease];
}

+ (id)serviceRequestMethod:(NSString *)strMethod
               SoapMessage:(NSString *)soapMsg forPress:(NSString *)press
{
    return [[[self alloc]initWithMethod:strMethod SoapMessage:soapMsg forPress:press]autorelease];
}
         ...
         SoapMessage:(NSString *)soapMsg
{
	...
	if ((self = [super init])) {
        ...
        [self.runningRequest addRequestHeader:@"SOAPAction" value:[NSString stringWithFormat:@"%@/%@",Base_NameSpace,strMethod]];
        [self.runningRequest setRequestMethod:@"POST"];
        //传soap信息
        [self.runningRequest appendPostData:[soapMessage dataUsingEncoding:NSUTF8StringEncoding]];
         ...
         SoapMessage:(NSString *)soapMsg forPress:(NSString *)press
{
	...
	if ((self = [super init])) {
        ...
        NSString *soapMessage = [NSString stringWithFormat:
                                 ...
                                 "</%@>\n"
                                 "</soap:Body>\n"
                                 "</soap:Envelope>\n",strMethod,press,soapMsg,press,strMethod
                                 ];
        NSString *msgLength = [NSString stringWithFormat:@"%d", [soapMessage length]];
        CLog(@"soapMsg is %@",soapMessage);
        //以下对请求信息添加属性前四句是必有的，第五句是soap信息。
        [self.runningRequest addRequestHeader:@"Host" value:[url host]];
        [self.runningRequest addRequestHeader:@"Content-Type" value:@"text/xml; charset=utf-8"];
        [self.runningRequest addRequestHeader:@"Content-Length" value:msgLength];
